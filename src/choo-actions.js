const DISPLAY = `DISPLAY`;
const isShownAddIdea = `${DISPLAY}: isShown add idea`;
const isShownAddProCon = `${DISPLAY}: isShown add procon`;

const COUNT = `COUNT`;
const countIncrement = `${COUNT}: increment`;
const countSet  = `${COUNT}: set`;

const IDEA = `IDEA`;
const ideaVoteIncrement = `${IDEA}: idea vote increment`;
const ideaProConAddition = `${IDEA}: idea procon addition`;
const ideaProConVote = `${IDEA}: idea procon vote`;
const ideaProConFormInputDescription = `${IDEA}: idea procon form input description`;
const ideaFormInputDescription = `${IDEA}: idea form input description`;

module.exports = {
  isShownAddIdea, isShownAddProCon,
  countIncrement, countSet,
  ideaVoteIncrement,
  ideaProConAddition, ideaProConVote,
  ideaProConFormInputDescription,
  ideaFormInputDescription,
}
